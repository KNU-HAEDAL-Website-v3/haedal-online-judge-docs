...
> 기준 규약: [cohort/design.md](../cohort/design.md) §3·§4 — 이 문서는 그 규약에 추가되는 결정만 기록
...
1. **권한 = 운영진 이상** — permissions.md §4: 수강생 배정·제외는 ADMIN(모든 반)·OPERATOR(자기 반). 운영진 지정/해제(ADMIN 전용)와 구분.
...
- `ApiTestSupport.enrollStudent`: 리포지토리 직접 저장 → **이 API 호출로 교체** (cohort/design.md §6 예정 항목)
